...
- `/addProject` has the same `#word` corruption bug but **ignores the
  ...
  `parentId` corrupted even with the header set). `create_category_or_project`
  therefore blocks `#` in project titles locally with an explanatory error.
  Category titles are safe — they go through `/doc/create`, which parses
  nothing.
